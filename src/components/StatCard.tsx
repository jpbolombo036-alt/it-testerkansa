...
export default function StatCard({ title, value, description, icon, panelClass }: StatCardProps) {
  return (
    ...
    >
      ...
      <p className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-500 sm:text-sm">{title}</p>
      <h3 className="mt-2 text-2xl font-semibold text-slate-900 sm:mt-4 sm:text-3xl">{value}</h3>
      <p className="mt-2 text-xs leading-6 text-slate-600 sm:mt-3 sm:text-sm">{description}</p>
    </motion.article>
  )
}
