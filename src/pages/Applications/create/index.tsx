...
import { createApplication, ApplicationCreateData } from '../../../api/applicationApi'
...
export default function ApplicationCreatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.nom.trim()) {
      showToast('error', 'Erreur', 'Le nom de l\'application est obligatoire.')
      return
    }
    try {
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Nouvelle application</h1>
            <p className="text-xs text-slate-500 dark:text-slate-400">Créez une nouvelle application dans le système</p>
          </div>
        </div>
      </div>
